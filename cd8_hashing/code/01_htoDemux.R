...
pbmc.hashtag <- HTODemux(pbmc.hashtag, assay = "HTO", positive.quantile = 0.999)
...
pbmc.hashtag.subset <- RunTSNE(pbmc.hashtag.subset, distance.matrix = hto.dist.mtx, perplexity = 200)
...
ggplot(confusion_df, aes(x = hash.ID, y = assignment, label = count, fill = count)) +
  geom_tile(color = "black") +
  geom_text() + pretty_plot() + scale_fill_gradientn(colors = jdb_palette("brewer_heat"))
