...
adtmat <- import_kite_counts("../data/LibA/")
colnames(adtmat) <- paste0(colnames(adtmat), "-1")

# Import the ATAC data
counts <- Read10X_h5(filename = "../../EF_PBMC_A_ATAC_hg38_v12-mtMask/outs/filtered_peak_bc_matrix.h5")
valid_bcs <- intersect(colnames(counts), colnames(adtmat))
adtmat <- adtmat[,valid_bcs]
counts <- counts[,valid_bcs]

# Slower but convenient for the row parsing
metadata <- read.csv(
  file = "../../EF_PBMC_A_ATAC_hg38_v12-mtMask/outs/singlecell.csv",
  header = TRUE,
  row.names = 1
)
...
CA <- CreateChromatinAssay(
  ...
  sep = c(":", "-"),
  genome = 'hg38',
  fragments = '../../EF_PBMC_A_ATAC_hg38_v12-mtMask/outs/fragments.tsv.gz',
  min.cells = 0,
  min.features = 0
)
